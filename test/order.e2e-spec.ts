...
describe('OrderController (e2e)', () => {
  ...
  // 주문 취소 테스트
  describe('/v1/orders/:id/cancel (POST)', () => {
    it('사용자는 자신의 주문을 취소할 수 있어야 함', () => {
      return request(app.getHttpServer())
        .post(`/v1/orders/${testOrder.id}/cancel`)
        .set('Authorization', `Bearer ${viewerToken}`)
        .send({ reason: '단순 변심' })
        .expect(404); // findOne이 null을 반환하므로 404 에러가 나올 것
    });
  ...
  // 배송 정보 업데이트 테스트
  describe('/v1/orders/:orderId/shipping-info (PATCH)', () => {
    ...
    it('판매자는 자신의 주문 상품 배송 정보를 성공적으로 업데이트해야 함 (200 OK)', async () => {
      return request(app.getHttpServer())
        .patch(`/v1/orders/${orderForShippingTest.id}/shipping-info`)
    ...
    it('존재하지 않는 주문 ID로 요청 시 404 Not Found 에러 발생', async () => {
      return request(app.getHttpServer())
        .patch('/v1/orders/non-existent-id/shipping-info')
        .set('Authorization', `Bearer ${sellerToken}`)
        .send(shippingDto)
        .expect(404);
    });
